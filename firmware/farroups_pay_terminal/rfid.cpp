...
void rfidIniciar() {
  ...
  for (int i = 0; i < NUM_LEITORES; i++) {
    ...
    Serial.printf("[RFID] Leitor %d (SS=%d) versao 0x%02X %s\n",
                  ...
                  (versao == 0x00 || versao == 0xFF) ? "<-- VERIFIQUE A FIACAO!" : "OK");
  }
}

#if USAR_TOKEN_CARTAO
// Le o token secreto gravado num bloco protegido do cartao.
// Retorna string vazia se nao conseguir autenticar/ler.
static String lerTokenCartao(MFRC522 &leitor) {
  ...
  if (status != MFRC522::STATUS_OK) {
    Serial.print(F("[RFID] Falha na autenticacao do cartao: "));
  ...
  if (status != MFRC522::STATUS_OK) {
    Serial.print(F("[RFID] Falha ao ler o bloco: "));
    Serial.println(MFRC522::GetStatusCodeName(status));
    return String();
  }
